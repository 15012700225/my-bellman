use std::cmp;

use crate::gpu::error::GPUError;
use crate::gpu::error::GPUResult;
use log::info;
use paired::bls12_381::{Bls12, Fr};
use rust_gpu_tools::call_kernel;
use rust_gpu_tools::opencl;

use crate::gpu::sources;
use crate::sector_id::SECTOR_ID;
use ff::Field;

#[repr(transparent)]
#[derive(Copy, Clone, Debug, PartialEq)]
struct OclFr(pub Fr);

impl Default for OclFr {
    fn default() -> Self {
        Self(Fr::zero())
    }
}

use rust_gpu_tools::opencl::Parameter;
unsafe impl Parameter for OclFr {}

pub struct FFTKernel {
    program: opencl::Program,
    pq_buffer: opencl::Buffer<Fr>,
    omegas_buffer: opencl::Buffer<Fr>,
}

impl FFTKernel {
    pub fn create(_: bool, gpu_index: usize) -> GPUResult<Self> {
        let devices = opencl::Device::all()?;
        if devices.is_empty() {
            return Err(GPUError::Simple("No working GPUs found!"));
        }

        // Select the specified device for FFT
        let device = devices[gpu_index].clone();

        let src = sources::kernel::<Bls12>(device.brand() == opencl::Brand::Nvidia);

        let program = opencl::Program::from_opencl(device, &src)?;
        let pq_buffer = program.create_buffer::<Fr>(1 << crate::gpu::MAX_LOG2_RADIX >> 1)?;
        let omegas_buffer = program.create_buffer::<Fr>(super::fft::LOG2_MAX_ELEMENTS)?;

        info!("{:?}: FFT: 1 working device(s) selected.", *SECTOR_ID);
        info!(
            "{:?}: FFT: Device 0: {}",
            *SECTOR_ID,
            program.device().name()
        );

        Ok(FFTKernel {
            program,
            pq_buffer,
            omegas_buffer,
        })
    }

    /// Peforms a FFT round
    /// * `log_n` - Specifies log2 of number of elements
    /// * `log_p` - Specifies log2 of `p`, (http://www.bealto.com/gpu-fft_group-1.html)
    /// * `deg` - 1=>radix2, 2=>radix4, 3=>radix8, ...
    /// * `max_deg` - The precalculated values pq` and `omegas` are valid for radix degrees up to `max_deg`
    fn radix_fft_round(
        &mut self,
        src_buffer: &opencl::Buffer<Fr>,
        dst_buffer: &opencl::Buffer<Fr>,
        log_n: u32,
        log_p: u32,
        deg: u32,
        max_deg: u32,
    ) -> GPUResult<()> {
        let n = 1u32 << log_n;
        let local_work_size = 1 << cmp::min(deg - 1, super::fft::MAX_LOG2_LOCAL_WORK_SIZE);
        let global_work_size = (n >> deg) * local_work_size;
        let kernel = self.program.create_kernel(
            "radix_fft",
            global_work_size as usize,
            Some(local_work_size as usize),
        );

        call_kernel!(
            kernel,
            src_buffer,
            dst_buffer,
            &self.pq_buffer,
            &self.omegas_buffer,
            opencl::LocalBuffer::<Fr>::new(1 << deg),
            n,
            log_p,
            deg,
            max_deg
        )?;

        Ok(())
    }

    /// Share some precalculated values between threads to boost the performance
    fn setup_pq_omegas(&mut self, omega: &Fr, n: usize, max_deg: u32) -> GPUResult<()> {
        // Precalculate:
        // [omega^(0/(2^(deg-1))), omega^(1/(2^(deg-1))), ..., omega^((2^(deg-1)-1)/(2^(deg-1)))]
        let mut pq = vec![Fr::zero(); 1 << max_deg >> 1];
        let twiddle = omega.pow([(n >> max_deg) as u64]);
        pq[0] = Fr::one();
        if max_deg > 1 {
            pq[1] = twiddle;
            for i in 2..(1 << max_deg >> 1) {
                pq[i] = pq[i - 1];
                pq[i].mul_assign(&twiddle);
            }
        }
        self.pq_buffer.write_from(0, &pq)?;

        // Precalculate [omega, omega^2, omega^4, omega^8, ..., omega^(2^31)]
        let mut omegas = vec![Fr::zero(); 32];
        omegas[0] = *omega;
        for i in 1..crate::gpu::LOG2_MAX_ELEMENTS {
            omegas[i] = omegas[i - 1].pow([2u64]);
        }
        self.omegas_buffer.write_from(0, &omegas)?;

        Ok(())
    }

    #[allow(clippy::too_many_arguments)]
    pub fn ifft_coset_fft(
        &mut self,
        a: &mut [Fr],
        omega: &Fr,
        log_n: u32,
        _minv: &Fr,
    ) -> GPUResult<()> {
        let n = 1 << log_n;
        let mut src_buffer = self.program.create_buffer::<Fr>(n)?;
        let mut dst_buffer = self.program.create_buffer::<Fr>(n)?;
        let max_deg = cmp::min(super::fft::MAX_LOG2_RADIX, log_n);
        self.setup_pq_omegas(omega, n, max_deg)?;
        src_buffer.write_from(0, &*a)?;

        let mut log_p = 0u32;
        while log_p < log_n {
            let deg = cmp::min(max_deg, log_n - log_p);
            self.radix_fft_round(&src_buffer, &dst_buffer, log_n, log_p, deg, max_deg)?;
            log_p += deg;
            std::mem::swap(&mut src_buffer, &mut dst_buffer);
        }

        // TODO: src_buffer multiply minv

        Ok(())
    }

    /// Performs FFT on `a`
    /// * `omega` - Special value `omega` is used for FFT over finite-fields
    /// * `log_n` - Specifies log2 of number of elements
    pub fn radix_fft(&mut self, a: &mut [Fr], omega: &Fr, log_n: u32) -> GPUResult<()> {
        let n = 1 << log_n;
        let mut src_buffer = self.program.create_buffer::<Fr>(n)?;
        let mut dst_buffer = self.program.create_buffer::<Fr>(n)?;

        let max_deg = cmp::min(super::MAX_LOG2_RADIX, log_n);
        self.setup_pq_omegas(omega, n, max_deg)?;

        src_buffer.write_from(0, &*a)?;

        let mut log_p = 0u32;
        while log_p < log_n {
            let deg = cmp::min(max_deg, log_n - log_p);
            self.radix_fft_round(&src_buffer, &dst_buffer, log_n, log_p, deg, max_deg)?;
            log_p += deg;
            std::mem::swap(&mut src_buffer, &mut dst_buffer);
        }

        src_buffer.read_into(0, a)?;

        Ok(())
    }
}
